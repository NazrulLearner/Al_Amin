...
const Receipt: React.FC = () => {
  ...
  useEffect(() => {
    if (id) {
      ...
      setError('রসিদ আইডি পাওয়া যায়নি');
      setLoading(false);
    }
  }, [id]);
  ...
  const fetchTransaction = async () => {
    try {
      ...
      const transactions = await feesService.getAllTransactions(1000);
      console.log('Total transactions:', transactions.length);
      
      const found = transactions.find(t => t.receiptId === id);
      
      if (found) {
        console.log('Transaction found:', found.receiptId);
        setTransaction(found);
      } else {
        console.log('Transaction NOT found for:', id);
        setError('রসিদটি পাওয়া যায়নি।');
      }
    } catch (error) {
      console.error('Error fetching transaction:', error);
      setError('রসিদ লোড করতে সমস্যা হয়েছে।');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getCollectorName = (transaction: FeeTransaction): string => {
    ...
    if (transaction.collectorId) return 'Collector';
    return 'System';
  };
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 flex justify-center items-center">
        <div className="text-center">
          ...
          <p className="text-xs text-gray-400 mt-2">রসিদ নং: {id}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 py-20 px-4">
        <div className="max-w-md mx-auto text-center">
          ...
          <p className="text-gray-500 mb-2">রসিদ নং: <span className="font-mono">{id}</span></p>
          <p className="text-gray-400 mb-6">{error || 'রসিদটি খুঁজে পাওয়া যায়নি।'}</p>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 py-8 px-4">
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex justify-between items-center mb-5 no-print">
          <button
            onClick={handleGoBack}
            className="flex items-center gap-2 px-4 py-2 bg-white text-gray-700 rounded-xl hover:bg-gray-100 shadow-md border border-gray-200"
          >
          ...
          <div className="flex gap-3">
            <button
              onClick={handlePrint}
              className="flex items-center gap-2 px-5 py-2 bg-blue-600 text-white rounded-xl hover:bg-blue-700 shadow-md"
            >
        ...
        <div className="receipt-container bg-white rounded-2xl shadow-2xl overflow-hidden relative">
          {watermarkEnabled && (
            ...
            >
              <div style={{ transform: `rotate(${watermarkRotation}deg)` }}>
                {orgLogo ? (
                  <img src={orgLogo} alt="Watermark" className="w-48 h-48 mx-auto opacity-30" style={{ filter: 'grayscale(100%)' }} />
                ) : (
                  <div className="text-center">
                    <div className="text-8xl font-bold text-gray-300">🏢</div>
                  </div>
                )}
                <div className="text-2xl font-bold text-gray-300 mt-2 whitespace-nowrap">{watermarkText}</div>
          ...
          <div className="p-6 relative z-10 bg-white">
            <div className="text-center border-b-2 border-dashed border-gray-200 pb-5 mb-5">
              {orgLogo && (
                <img src={orgLogo} alt="Logo" className="h-20 w-20 mx-auto mb-2 object-contain rounded-full shadow-md" />
              )}
              <h1 className="text-2xl font-bold text-gray-800 tracking-tight">{orgName}</h1>
              {orgAddress && <p className="text-xs text-gray-500 mt-1 max-w-md mx-auto">{orgAddress}</p>}
            ...
            <div className="grid grid-cols-2 gap-4 mb-5 pb-3 border-b border-gray-200">
              <div className="bg-gray-50 rounded-lg p-3 border-l-4 border-emerald-500">
                ...
                <p className="text-lg font-mono font-bold text-gray-800 mt-1">{transaction.receiptId}</p>
              </div>
              <div className="bg-gray-50 rounded-lg p-3 border-l-4 border-blue-500 text-right">
                ...
                <p className="text-lg font-semibold text-gray-800 mt-1">{formatDateWithSettings(transaction.createdAt, dateFormat)}</p>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 mb-4 border border-blue-200">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="text-base font-semibold text-gray-800 mt-0.5">{transaction.memberName}</p>
                </div>
                <div>
                  <p className="text-xs text-blue-600">সদস্য আইডি</p>
                  <p className="text-base font-mono font-semibold text-gray-800 mt-0.5">{transaction.memberId}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl p-4 mb-4 border border-emerald-200">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-800 mt-0.5">{transaction.paymentPeriod}</p>
                </div>
                <div>
                  <p className="text-xs text-emerald-600">মাসের সংখ্যা</p>
                  <p className="text-sm font-medium text-gray-800 mt-0.5">{transaction.monthsPaid} মাস</p>
                </div>
                <div>
                  <p className="text-xs text-emerald-600">পেমেন্ট পদ্ধতি</p>
                  <p className="text-sm font-medium text-gray-800 mt-0.5">{getPaymentMethodName(transaction.payType)}</p>
                </div>
                {transaction.referenceNo && (
                  <div>
                    <p className="text-xs text-emerald-600">ট্রানজেকশন আইডি</p>
                    <p className="text-xs font-mono text-gray-700 break-all mt-0.5">{transaction.referenceNo}</p>
                  </div>
                )}
              </div>
            </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {getDepositBankName(transaction) && (
                    <div>
                      ...
                      <p className="text-sm font-medium text-gray-800 mt-0.5">{getDepositBankName(transaction)}</p>
                    </div>
                  )}
                  {getDepositReference(transaction) && (
                    <div>
                      <p className="text-xs text-purple-600">রেফারেন্স</p>
                      <p className="text-xs font-mono text-gray-700 break-all mt-0.5">{getDepositReference(transaction)}</p>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="text-center py-4 mb-4 border-y-2 border-dashed border-gray-200 bg-gradient-to-r from-amber-50 to-yellow-50 rounded-lg">
              <p className="text-xs text-amber-700 font-semibold uppercase tracking-wide mb-2">মোট পরিশোধিত পরিমাণ</p>
              <p className="text-3xl font-bold text-green-600">{formatCurrencyWithSettings(transaction.feeAmount, financialSettings)}</p>
              <p className="text-xs text-gray-500 mt-2">কথায়: {convertToBengaliWords(transaction.feeAmount)}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4 pt-3 border-t border-gray-200">
              <div className="bg-gradient-to-r from-cyan-50 to-sky-50 rounded-lg p-3 border border-cyan-200">
                ...
                <p className="text-base font-medium text-gray-800 mt-1">{getCollectorName(transaction)}</p>
              </div>
              <div className="bg-gradient-to-r from-orange-50 to-amber-50 rounded-lg p-3 border border-orange-200">
                <p className="text-xs text-orange-600 font-semibold text-right">প্রবেশকারী</p>
                <p className="text-base font-medium text-gray-800 mt-1 text-right">{getEnteredByName(transaction)}</p>
              </div>
            </div>
            ...
            <div className="text-center pt-4 border-t-2 border-gray-200 mt-2">
              {(transaction.receiptFooter || settings?.collection?.receiptFooter) && (
                <p className="text-xs text-gray-500 italic mb-2">{transaction.receiptFooter || settings?.collection?.receiptFooter}</p>
              )}
              <p className="text-xs text-gray-400">© {new Date().getFullYear()} {orgName} | সফটওয়্যার জেনারেটেড রসিদ</p>
              <p className="text-xs text-gray-400 mt-1">প্রিন্ট: {formatDateTime(new Date())}</p>
            </div>
          </div>
